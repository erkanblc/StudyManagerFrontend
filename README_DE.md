...
Dieses Repository enthält das **React-Frontend** (`StudyManagerFrontend`). Die REST-API liegt im benachbarten Verzeichnis [`StudyManagerBackend`](../StudyManagerBackend) / [`backend`](../backend).

> Andere Sprachen: [English](README.md) · [Türkçe](README_TR.md)  
> Architekturdiagramme: [`architecture-uml.md`](architecture-uml.md)
...
- Automatische Token-Erneuerung über Axios-Interceptor (`/api/auth/refresh`)
...
Während der Entwicklung leitet Vite `/api`-Anfragen an `http://127.0.0.1:8080` weiter und vermeidet so CORS-Probleme auf dem Client.

Detaillierte UML (Komponente, Sequenz, Klassenstil, Deployment): siehe [`architecture-uml.md`](architecture-uml.md).
...
Datei `src/main/resources/application.properties` im Backend bearbeiten:
...
```bash
cd ../StudyManagerBackend   # oder ../backend — je nach Ordnername
...
Aus dem Wurzelverzeichnis dieses Repositories (`StudyManagerFrontend`):
...
```
EducationPlatform/
├── StudyManagerBackend/              # Spring Boot REST API (Schwester)
│   └── src/main/java/.../
│       ├── config/
│       ├── controller/
│       ├── dto/
...
- Frontend (Türkisch): [`README_TR.md`](README_TR.md)
- Architektur-UML: [`architecture-uml.md`](architecture-uml.md)
- Backend-README (falls im Schwester-Repository vorhanden)
